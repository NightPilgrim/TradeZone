(function () {
    ...
    const ASSETS = ['BTC/USD', 'ETH/USD', 'EUR/USD', 'XAU/USD', 'XAG/USD'];
    ...
    function migrateDepositToDeposits() {
        const oldVal = localStorage.getItem('tj_deposit');
        if (oldVal === null) return;
        const n = parseFloat(oldVal);
        if (isNaN(n) || n <= 0) {
            localStorage.removeItem('tj_deposit');
            return;
        }
        const deposits = [{
            id: 'd' + Date.now(),
        ...
        localStorage.setItem(storageKey('tj_deposits'), JSON.stringify(deposits));
        localStorage.removeItem('tj_deposit');
    }
    ...
    function getDeposits() {
        ...
        try {
            const raw = localStorage.getItem(storageKey('tj_deposits'));
    ...
    function getStoragePrefix() {
        ...
        return id != null ? String(id) : 'local';
    }

    function storageKey(name) {
        return name + '_' + getStoragePrefix();
    }
    ...
    function getTrades() {
        ...
        try {
            const raw = localStorage.getItem(storageKey('tj_trades'));
            const arr = raw ? JSON.parse(raw) : [];
            return Array.isArray(arr) ? arr : [];
        } catch (_) {
            return [];
    ...
    function saveTrades(trades) {
        if (viewOnlyMode) return;
        localStorage.setItem(storageKey('tj_trades'), JSON.stringify(trades));
    }
